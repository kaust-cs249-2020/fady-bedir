'''
Implement GreedyMotifSearch after applying la place rule when building the profiles
...
file = open('/users/fadygamal/lagreed.txt', 'r')
text = file.read()
lines = text.split('\n')
...
def greedylaplace(k, t, dna): 
    ...
    for i in range (len(dna[0]) - k + 1): #iterate through each k pattern in the first string in dna
        ...
        for j in range(1, t):                   #iterate through each other string in dna to
            profile = laplace_prof_matrix(motifs, k)    #form a probability profile form the motifs matrix above
